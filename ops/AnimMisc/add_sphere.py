...
def create_bounding_balls(armature_obj, report=None):
    ...
    material_name = f"bb_material_{nama_char}"
    ...
    try:
        ...
        for pos_bone_name, mesh_name_tmpl, constraint_name, target_bone_name, scale in BB_MAP:
            ...
            if pose_bone is None:
                msg = f"[SKIP] Bone posisi '{pos_bone_name}' tidak ditemukan, dilewati."
                print(msg)
                if report:
                    report({'WARNING'}, msg)
                continue
            ...
            if target_pose_bone is None:
                msg = (f"Target bone '{target_bone_name}' untuk constraint "
